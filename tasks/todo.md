...
- [~] NORTE design system - shell slice: Topbar (18.7) + ThemeToggle (light/dark, inline SVG, no new dep) + Button/Card primitives (tokens); wired into /panel (Card metrics) and /mi-espacio (both views). Pending: migrate screen internals (tables/badges/forms/auth) off default Tailwind palette to tokens; adopt lucide-react for the broader icon set.
