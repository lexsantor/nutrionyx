...
- [~] Token migration batch 1: Input primitive; auth (sign-in/up), crear-consulta, /panel (table, badges, invite, cancel), home, logout off default palette to NORTE tokens. Build + visual green (owner). Pending batch 2: accept-invitation flow + evaluacion wizard (wizard-step, review, guardrail-banner). Then adopt lucide-react for the broader icon set.
