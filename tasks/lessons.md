...
- **Outdated 2026-08-14, the @swc/helpers rule below.** It came from next-intl
  declaring `@swc/helpers` and npm nesting `0.5.23` beside next's `0.5.15`.
  next-intl 4.13.1 declares no such dependency, nothing is nested on disk, and
  a regenerated lockfile that omits it passes a REAL `npm ci` (exit 0). Verified
  when adding `@vercel/analytics`. Keep the habit - check the lockfile after any
  dependency change, and verify with a real `npm ci`, never a grep for one
  package name that a version bump can retire.
