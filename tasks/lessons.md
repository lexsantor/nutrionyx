...
- **2026-08-14: the @swc/helpers rule below is NOT outdated, and I broke CI
  twice proving it.** Adding `@vercel/analytics` dropped
  `next-intl/node_modules/@swc/helpers@0.5.23`; a from-scratch
  `npm install --package-lock-only` did not restore it; `next-intl@4.13.1`
  declares no `@swc/helpers` and none is nested on disk; and a REAL `npm ci` on
  macOS passed at exit 0. Every one of those observations is true and the
  conclusion drawn from them - "the entry is stale" - was false. CI's Linux
  `npm ci` failed with `Missing: @swc/helpers@0.5.23 from lock file`, because
  the requirement comes from a platform-optional the macOS resolve never sees.
  **A local `npm ci` cannot clear a lockfile; only Linux CI can.** The fix that
  worked: `git checkout <last-green-commit> -- package-lock.json`, then
  `npm install <pkg> --package-lock-only`, then splice back by hand the nodes
  it dropped (here six: the nested helper and five `@next/swc-*` optionals)
  until the diff against the green lockfile is exactly the package being added.
